...
This uses one preemptible eight-H100 node, with no LLM, router, CPU utility node or remote GPU. The existing $50 cumulative authorization and cost history remain in force. Reserve $13 before admitting a run; the initial attempt used a 20/30-minute cleanup/deletion policy. Rayhan then authorized 10 extra minutes: subsequent diagnostic attempts use 30/40 minutes, at most about $11.48 at the full hourly rate before tax. Placement timeout stays 15 minutes. Existing saved sessions retain their deadlines. The local guard is not a provider-enforced billing cap.

## Launch

First confirm no active attempt and successful cleanup/cost records for prior attempts. Refresh capacity advice for the selected eu-north1 fabric. The September 16 explicit execute-now instruction permitted a single bounded native placement probe despite an older low advisory sample; native placement succeeded. A follow-up may reuse that proven placement after cleanup under the existing multiple-attempt authorization. Stop promptly on native capacity rejection; do not repeatedly retry unchanged negative advice. Generate a fresh plan with `ipc_diagnostic_only=true`, `gpu_preemptible=true` and the selected fabric. The plan must contain exactly the Kubernetes cluster, GPU cluster, and one local eight-H100 node: three creates, no other changes.

Use the private approval record `/Users/rayhanmemon/.codex/run-state/router-h100-pilot/approval-ipc-diagnostic-extended-2026-09-16.json`:

```sh
python3 infra/topology/pilot-session.py --execute --profile ipc-diagnostic \
  --approval-record /Users/rayhanmemon/.codex/run-state/router-h100-pilot/approval-ipc-diagnostic-extended-2026-09-16.json \
...
python3 infra/topology/run-ipc-diagnostic.py --context router-topology \
  --node ACTUAL_NODE --run-dir RUN_DIR --split-first --execute
```
...
The retry uses `--split-first`: test separate Pods first, then rerun same-container controls on that exact physical GPU pair. GPU enumeration masking is tested if the split case fails. Pod deletion is reconciled until actual absence, bounded by the saved deadline. Cases stop adaptively after an earlier control failure. The runner preserves time for shutdown and does not extend the guard deadline. A failed case is diagnostic evidence, not an infrastructure failure to discard. It does not prove UCX/vLLM will choose a particular transport. If raw CUDA succeeds, review whether time remains for the existing small PyTorch/NIXL follow-up; do not start full model serving in this profile.
