...
def default_cleanup(run):
    subprocess.run([sys.executable, str(HERE / "pilot-session.py"),
                    "cleanup", str(run), "--execute"], check=True)


def main(argv=None, serving_module=None, cleanup=default_cleanup):
    m = serving_module or globals()["m"]
    args=m.parse_args(argv);run=args.run_dir;s=m.read_json(run/'session.json')
